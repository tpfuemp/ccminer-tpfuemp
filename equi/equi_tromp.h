...
// Adapted for ccminer: uses the fork's eq_blake2b_* host impl (blake2/blake2.h
// is included by the wrapper TU before this header); tromp's own blake include
// dropped to avoid its SSE-only blake2b.cpp (MSVC lacks __SSE2__) and symbol
// clashes with sph/blake2b.
...
#elif defined(_WIN32)
#define htole32(x) (x) /* Windows/x86 is little-endian */
#else
#include <endian.h>
#endif
...
void setheader(blake2b_state *ctx, const char *headernonce, const char *personal) {
  ...
  eq_blake2b_update(ctx, (const uint8_t *)headernonce, HEADERNONCELEN);
}
...
void genhash(const blake2b_state *ctx, u32 idx, uchar *hash) {
  ...
  eq_blake2b_update(&state, (uint8_t *)&leb, sizeof(u32));
  uchar blakehash[HASHOUT];
  eq_blake2b_final(&state, blakehash, HASHOUT);
