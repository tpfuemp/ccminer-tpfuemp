...
/*
 * Shared BMW-512 primitive library (device side) — x-family stage function.
 *
 * First-round compression (bmw512_round1) and the fully-unrolled final
 * compression extracted bit-identically from quark/cuda_bmw512.cu (based
 * on SP's BMW kernel, Provos Alexis 2016, sp pascal tuning May 2018).
 ...
 *
 * The 80-byte first-stage kernels, their Compression512 helper and the
 * midstate precompute stay in the consuming file.
 *
 * Interface convention: msg[16] carries the 64-byte value in msg[0..7]
 * exactly as it sits in the inter-stage d_hash buffer (little-endian
 * uint64 words; BMW is little-endian native). The digest lands in
 * msg[8..15]; msg is clobbered.
 */

#ifndef CUDA_BMW512_DEVICE_CUH
#define CUDA_BMW512_DEVICE_CUH

#include <stdint.h>
#include <cuda_helper_alexis.h>  // ROL2/SHL2/SHR2/ROTL64/xor3x/vectorize/devectorize
...
/* BMW-512 of a 64-byte input: msg[0..7] = input (d_hash word order),
 * digest lands in msg[8..15] (in place; msg is clobbered). Body is the
 * donor hash_64 kernel's final compression, verbatim. */
__device__ __forceinline__
void bmw512_hash_64(uint64_t *msg)
{
	uint2 h[16];
	uint2 q[32];
...
/* Word-3 projection for target-compare-only final stages: same compression,
 * but only digest words needed for msg[11] (bytes 24..31, the word the
 * 64-bit target compare reads) are computed — the donor final kernel's
 * exact text. Never feed this into another hash stage or the submit path;
 * the scanhash CPU re-verify stays authoritative. */
__device__ __forceinline__
uint64_t bmw512_hash_64_word3(uint64_t *msg)
{
	uint2 h[16];
	uint2 q[32];
