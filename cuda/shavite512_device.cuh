/*
 * SHAvite-512 shared device library (docs/coding-guideline.md §3).
 *
 * c512 needs the 1024-word shared AES table: kernels declare
 * __shared__ uint32_t sharedMemory[1024], call shavite_gpu_init(sharedMemory)
 * (cooperative, threads < 128) and MUST __syncthreads() before hashing —
 * the shavite/echo shared-AES-table race lesson.
 *
 * Extracted verbatim from x11/cuda_x11_shavite512.cu. NOTE: this formulation
...
#define CUDA_SHAVITE512_DEVICE_CUH

#include <cuda_helper.h>

#define AESx(x) (x ##UL) /* SPH_C32(x) */

__device__ __align__(64) uint32_t d_shavite_AES0[256] = {
...
__constant__ __align__(64) uint32_t d_shavite_AES1[256] = {
...
__constant__ __align__(64) uint32_t d_shavite_AES2[256] = {
...
__constant__ __align__(64) uint32_t d_shavite_AES3[256] = {
...
uint32_t &y0, uint32_t &y1, uint32_t &y2, uint32_t &y3)
{
	...
	y3 = xor4_32(
		__ldg(&d_shavite_AES0[x3 & 0xff]),  //sharedMemory[__byte_perm(x3, 0, 0x4440)],
...
uint32_t &x0, uint32_t &x1, uint32_t &x2, uint32_t &x3)
{
	...
	shavite_aes_round(sharedMemory,
		x0, x1, x2, x3,
		y0, y1, y2, y3);
...
void shavite_gpu_init(uint32_t *sharedMemory)
{
	...
	if (threadIdx.x < 128) {
		sharedMemory[threadIdx.x] = d_shavite_AES0[threadIdx.x];
		sharedMemory[threadIdx.x + 256] = d_shavite_AES1[threadIdx.x];
		sharedMemory[threadIdx.x + 512] = d_shavite_AES2[threadIdx.x];
		sharedMemory[threadIdx.x + 768] = d_shavite_AES3[threadIdx.x];

		sharedMemory[threadIdx.x + 64 * 2] = d_shavite_AES0[threadIdx.x + 64 * 2];
		sharedMemory[threadIdx.x + 64 * 2 + 256] = d_shavite_AES1[threadIdx.x + 64 * 2];
		sharedMemory[threadIdx.x + 64 * 2 + 512] = d_shavite_AES2[threadIdx.x + 64 * 2];
		sharedMemory[threadIdx.x + 64 * 2 + 768] = d_shavite_AES3[threadIdx.x + 64 * 2];
	}
}


#endif /* CUDA_SHAVITE512_DEVICE_CUH */
