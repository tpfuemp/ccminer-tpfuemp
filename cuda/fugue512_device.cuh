 ...
 * per block).
 *
 * Usage: the caller declares __shared__ uint32_t mixtabs[1024]; calls
 * fugue512_load_shared(mixtabs) (which ends with __syncthreads()), then
 * fugue512_hash_64(mixtabs, Hash).
 *
 * NOTE: TIX4/CMIX36/SMIX and the mixtab0..3 access macros stay exported.
 *
...
#ifndef CUDA_FUGUE512_DEVICE_CUH
#define CUDA_FUGUE512_DEVICE_CUH
...
static __constant__ uint32_t c_fugue_mixtab0[256] = {
...
/* Finalization variant: absorbs a 4th input group (w,x,y,z) — used by the
 * 80-byte header path (FUGUE512_3 covers a 3-input group). */
...
uint32_t FUGUE_ROL8(const uint32_t a) {
	return __byte_perm(a, 0, 0x2103);
}
__device__ __forceinline__
uint32_t FUGUE_ROR8(const uint32_t a) {
	return __byte_perm(a, 0, 0x0321);
}
__device__ __forceinline__
uint32_t FUGUE_ROL16(const uint32_t a) {
	return __byte_perm(a, 0, 0x1032);
}
#else
#define FUGUE_ROL8(u)  ROTL32(u, 8)
#define FUGUE_ROR8(u)  ROTR32(u, 8)
#define FUGUE_ROL16(u) ROTL32(u,16)
#endif
...
/* Cooperative fill of the 4 rotated mixtabs into shared memory; ends with
 * __syncthreads() (the shavite/echo shared-AES-table race lesson). */
__device__ __forceinline__
void fugue512_load_shared(uint32_t *mixtabs)
{
	const uint32_t thr = threadIdx.x & 0xFF;
	const uint32_t tmp = c_fugue_mixtab0[thr];
	mixtabs[thr]       = tmp;
	mixtabs[thr + 256] = FUGUE_ROR8(tmp);
	mixtabs[thr + 512] = FUGUE_ROL16(tmp);
	mixtabs[thr + 768] = FUGUE_ROL8(tmp);
	if (blockDim.x < 256) {
		const uint32_t thr2 = (threadIdx.x + 0x80) & 0xFF;
		const uint32_t tmp2 = c_fugue_mixtab0[thr2];
		mixtabs[thr2]       = tmp2;
		mixtabs[thr2 + 256] = FUGUE_ROR8(tmp2);
		mixtabs[thr2 + 512] = FUGUE_ROL16(tmp2);
		mixtabs[thr2 + 768] = FUGUE_ROL8(tmp2);
	}
	__syncthreads();
}

/* Fugue-512 of a 64-byte input, in place, d_hash word order in and out —
 * the donor x13_fugue512_gpu_hash_64 kernel's interior, verbatim. */
__device__ __forceinline__
void fugue512_hash_64(const uint32_t *mixtabs, uint32_t *Hash)
{
	#pragma unroll 16
	for (int i = 0; i < 16; i++)
...
#endif /* CUDA_FUGUE512_DEVICE_CUH */
