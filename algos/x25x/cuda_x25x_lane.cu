/**
 * LANE-512 stage for X25X (SUQA/SIN).
 *
 * One thread per hash: consumes a 64-byte input slot (hash[22] in the chain)
 * and writes the 512-bit (64-byte) LANE digest into the slot.
 *
 * The tables (T0..T3, C, iv512) and lane512_compress() below are copied
 * verbatim from algos/x25x/lane.c (Indesteege's optimised ANSI-C LANE) with
 * only __device__ qualifiers added, so the compression stays bit-identical to
 * the CPU reference. laneHash(512) of a 64-byte message reduces to two
 * compress calls (data block padded to 128B, ctr=512; then the length-only
 * output-transform block, ctr=0), emitting h[0..15] big-endian.
 */
...
#define B2(x) (T8((x) >> 16))
#define B3(x) (T8((x) >> 24))
#define U8TO32_BIG(c)  (((uint32_t)T8(*((const uint8_t*)(c)))      << 24) | \
                        ((uint32_t)T8(*(((const uint8_t*)(c)) + 1)) << 16) | \
                        ((uint32_t)T8(*(((const uint8_t*)(c)) + 2)) <<  8) | \
                        ((uint32_t)T8(*(((const uint8_t*)(c)) + 3))))

__device__ static const uint32_t iv512[16] = {
...
__device__ static const uint32_t T0[256] = {
...
__device__ static const uint32_t T1[256] = {
...
__device__ static const uint32_t T2[256] = {
...
__device__ static const uint32_t T3[256] = {
...
__device__ static const uint32_t C[768] = {
...
__device__ static __noinline__ void lane512_compress(const uint8_t m[128], uint32_t h[16], const uint32_t ctrh, const uint32_t ctrl)
{
...
// laneHash(512) of exactly 64 bytes -> 64 bytes, in place on a d_hash slot.
__device__ __forceinline__
static void lane512_hash_64(uint8_t *slot /* 64 bytes, read+write */)
{
	uint32_t h[16];
	#pragma unroll
	for (int i = 0; i < 16; i++) h[i] = iv512[i];

	uint8_t m[128];
	#pragma unroll
	for (int k = 0; k < 64; k++)  m[k] = slot[k];
	#pragma unroll
	for (int k = 64; k < 128; k++) m[k] = 0;

	lane512_compress(m, h, 0, 512);       // MSB32(512)=0, LSB32(512)=512

	// output transformation block: flag 0x00 + 64-bit big-endian length (512 bits)
	#pragma unroll
	for (int k = 0; k < 128; k++) m[k] = 0;
	m[7] = 0x02;                          // (512 >> 8) & 0xff; all other length bytes are 0
	lane512_compress(m, h, 0, 0);

	#pragma unroll
	for (int i = 0; i < 16; i++) {
		slot[4*i + 0] = (uint8_t)(h[i] >> 24);
		slot[4*i + 1] = (uint8_t)(h[i] >> 16);
		slot[4*i + 2] = (uint8_t)(h[i] >>  8);
		slot[4*i + 3] = (uint8_t)(h[i]      );
	}
}

__global__ void x25x_lane_gpu_hash_64(uint32_t threads, uint64_t *g_hash)
{
	const uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
		lane512_hash_64((uint8_t*)&g_hash[thread << 3]);
}

__host__ void x25x_lane_cpu_hash_64(int thr_id, uint32_t threads, uint32_t *d_hash)
{
	const uint32_t threadsperblock = 128;
	dim3 grid((threads + threadsperblock - 1) / threadsperblock);
	dim3 block(threadsperblock);

	x25x_lane_gpu_hash_64<<<grid, block>>>(threads, (uint64_t*)d_hash);
}
