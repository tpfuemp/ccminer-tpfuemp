...
// This vendored reference is compiled scalar-only: the AVX2/SSE4.1/NEON paths
// depend on cpuminer-opt's simd-utils.h (not vendored). Force the pure-C
// fallback branches regardless of the host's -march so the file builds
// everywhere; the SWIFFTX GPU kernel is where the speed will come from.
#undef __AVX2__
#undef __SSE4_1__
#undef __ARM_NEON
#undef SIMD512
...
swift_int16_t multipliers[N] SWIFFTX_ALIGN64;
...
swift_int16_t fftTable[256 * EIGHTH_N] SWIFFTX_ALIGN64;
...
const swift_int16_t As[3 * M * N] SWIFFTX_ALIGN64 =
...
{
	int i, j;
	swift_int32_t result[N] SWIFFTX_ALIGN64;
						  ...
						  bool doSmooth)
{
	...
	swift_int32_t fftOut[N * M] SWIFFTX_ALIGN64;
	unsigned char intermediate[N * 3 + 8] SWIFFTX_ALIGN64;
...
void ComputeSingleSWIFFTX( unsigned char *input, unsigned char *output )
{
   ...
   // Will store the result of the FFT parts:
   swift_int32_t fftOut[N * M] SWIFFTX_ALIGN64;
   unsigned char sum[ N*3 + 8 ] SWIFFTX_ALIGN64;
   unsigned char carry0,carry1,carry2;
