...
// SWIFFTX's own integer aliases. NOTE: swift_int16_t / swift_uint16_t are
// deliberately 32-bit wide (the reference stores Z_257 residues in 32-bit
// slots); the "16" only reflects the logical field, not the storage size.
// Upstream carries these in a filename-shadowing "stdint.h"/"inttypes.h"
// pair; inlined here to avoid putting a file named stdint.h on the include path.
...
// Portable 64-byte alignment. GCC/Clang honour it; MSVC has no __attribute__,
// and the scalar reference path we compile only treats it as a perf hint, so
// dropping it there is harmless.
#if defined(_MSC_VER)
#define SWIFFTX_ALIGN64
#else
#define SWIFFTX_ALIGN64 __attribute__ ((aligned (64)))
#endif
