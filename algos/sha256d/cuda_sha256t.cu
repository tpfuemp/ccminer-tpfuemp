...
#define SHA256T_TPB 256
#define SHA256T_NPT 8 // sequential nonces per thread (donor-style)

__global__ __launch_bounds__(SHA256T_TPB)
void sha256t_gpu_hash_shared(const uint32_t threads, const uint32_t startNonce, uint32_t *resNonces)
{
	const uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);

	for (uint32_t i = 0; i < SHA256T_NPT; i++)
	{
		const uint32_t idx = thread * SHA256T_NPT + i;
		...
		sha256_transform_full(dat, buf, c_sha256_K);
...
{
	const uint32_t threadsperblock = SHA256T_TPB;

	dim3 grid((threads + threadsperblock*SHA256T_NPT - 1) / (threadsperblock*SHA256T_NPT));
