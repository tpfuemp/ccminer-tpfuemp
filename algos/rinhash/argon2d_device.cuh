...
// fBlaMka: match the C reference implementation exactly
...
// Blake2b G function - match the reference implementation exactly
...
// fill_block: match the C reference implementation exactly
...
// index_alpha: match the C reference implementation exactly (including the function signature)
...
// fill_segment added (matches the C reference implementation exactly)
...
// fill_memory: match the C reference implementation exactly
...
// blake2b_long: match the C reference implementation exactly
...
// device_argon2d_hash: match the C reference implementation exactly
...
//=== __global__ example kernel (salt-specified version) ===//
// Assumes the host pre-allocates block memory and passes its pointer (memory_ptr).
__global__ void argon2d_hash_device_kernel(
    uint8_t* output,
    const uint8_t* input, size_t input_len,
    uint32_t t_cost, uint32_t m_cost, uint32_t lanes,
    block* memory_ptr,   // pointer to the memory region allocated on the host
    const uint8_t* salt, size_t salt_len
) {
