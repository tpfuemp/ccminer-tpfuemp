/*
 * Fused multi-stage kernel for the x16 family (O1): executes a run of
 ...
 * (x16rv2), which gets a kernel variant with its 6KB shared table.
 *
 ...
 *
 * Stage ids match enum Algo in x16r.cu / x16rv2.cu; 16 = tiger192.
 */
...
void x16_fused_stage(const int id, uint64_t *const s, const uint64_t *sharedMem)
{
	switch (id) {
	case 0: /* BLAKE */
	...
	case 15: /* SHA512 */
		sha512_hash_64(s);
		break;
	case X16_FUSED_TIGER: { /* 24-byte digest, zero-padded to 64 */
		uint64_t buf[3];
...
void x16_fused_gpu_hash_64(const uint32_t threads, uint64_t *g_hash, const int start, const int len)
{
	...
	if (thread < threads)
	{
		...
		*(uint2x4*)&s[4] = __ldg4((uint2x4*)&pHash[4]);
		...
		for (int i = 0; i < len; i++)
			x16_fused_stage(c_fused_order[start + i], s, NULL);
...
__global__ __launch_bounds__(TPB_FUSED, 2)
void x16_fused_gpu_hash_64_tiger(const uint32_t threads, uint64_t *g_hash, const int start, const int len)
{
	...
	if (thread < threads)
	{
		...
		*(uint2x4*)&s[4] = __ldg4((uint2x4*)&pHash[4]);
		...
		for (int i = 0; i < len; i++)
			x16_fused_stage(c_fused_order[start + i], s, sharedMem);
...
void x16_fused_cpu_hash_64(int thr_id, uint32_t threads, int start, int len, int has_tiger, uint32_t *d_hash)
{
	...
	if (has_tiger)
		x16_fused_gpu_hash_64_tiger <<<grid, block>>> (threads, (uint64_t*)d_hash, start, len);
	else
		x16_fused_gpu_hash_64 <<<grid, block>>> (threads, (uint64_t*)d_hash, start, len);
}
