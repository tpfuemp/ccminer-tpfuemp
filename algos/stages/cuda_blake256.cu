...
static void blake256_compress2nd(uint32_t *h, const uint32_t *block, const uint32_t T0)
{
	...
	// sigma baked as literals so m[] stays register-resident: a runtime index into
	// a local array forces it to local memory. Same schedule and macro as the
	// fused blakeKeccak kernel.
	/* round 0 */
