/**
 * C11 (Flax / Chaincoin) algorithm — fixed 11-stage chain, reordered x11
 *
 ...
 * atomicExch chain) on modern arch — so there is no separate cuda_check_hash/
 * suppl pass; the arch<500 compat path falls back to plain echo + cuda_check_hash.
 */
