/**
 * X14 algorithm (fixed 14-stage chain) — x13 + Shabal-512
 *
 * blake - bmw - groestl - skein - jh - keccak - luffa - cubehash -
 * shavite - simd - echo - hamsi - fugue - shabal
 *
...
extern "C" int scanhash_x14(int thr_id,  struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	do {
		...
		if (use_compat_kernels[thr_id]) {
			...
			/* shabal terminal + on-device target compare (2 nonces via an atomicExch
			 * chain), eliding the shabal store + the cuda_check_hash/suppl passes */
			shabal512_cpu_hash_64_final(thr_id, throughput, d_hash[thr_id], d_resNonce[thr_id], AS_U64(&ptarget[6]));
